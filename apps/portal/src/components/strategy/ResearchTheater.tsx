...
const STRATEGY_STAGES = [
  { code: "RA", label: "analyst",    jobTypes: ["auto-pull"] },
...
export default function ResearchTheater({ projectId }: { projectId: string }) {
    ...
    <div className="bg-bg-card border border-border rounded-lg overflow-hidden">
      ...
      <div className="px-6 pt-5 pb-3 border-b border-white/[0.06]">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="font-mono text-[10px] tracking-[4px] lowercase text-[#8B9A6B]/70 mb-1">
              live research
            </p>
            ...
            return (
              <div
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-[3px] border transition-all duration-300 ${
                  s === "active"
                    ? "border-[#8B9A6B]/40 bg-[rgba(139,154,107,0.08)] text-[#8B9A6B]"
                    : s === "done"
                ...
                <span className="relative flex h-1.5 w-1.5 flex-shrink-0">
                  {s === "active" ? (
                    <>
                      ...
                      <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-[#8B9A6B]" />
                    </>
                  ) : s === "done" ? (
      ...
      {runningJob && activePersona && runningJob.progress && (
        <div className="px-6 py-3 border-b border-white/[0.06]">
          <div className="flex items-center gap-2 mb-2">
            <span className="font-mono text-[10px] text-[#8B9A6B]/80 tracking-[1px]">
              {activePersona.code}
            </span>
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 h-1 bg-white/[0.06] rounded-full overflow-hidden">
              <div
                className="h-full bg-[#8B9A6B]/50 rounded-full transition-all duration-700 ease-out"
                style={{
                  width: `${Math.round(
      ...
      <div ref={logPanelRef} className="px-6 py-4 max-h-[180px] overflow-y-auto">
        {log.length === 0 ? (
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[#8B9A6B]/30 animate-pulse" />
          ...
          <div className="space-y-px">
            {log.map((entry) => (
              ...
              >
                ...
                <span className="text-[#8B9A6B]/50">{entry.persona}:</span>{" "}
                {entry.text}
              </p>
            ))}
          </div>
